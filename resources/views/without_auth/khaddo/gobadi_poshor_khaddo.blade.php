...
<div class="section-home our-causes fadeIn">
    ...
    <div class="container">
        ...
        <h2 class="title-style-1" style="margin-top: 110px;">গবাদি পশুর খাদ্য <span class="title-under"></span></h2>

        <div class="row">
            @foreach($gobadiPoshorKhaddos as $key=>$macherKhaddo)
                <div class="col-md-2 col-sm-6">

                    <div class="cause fbbox">
                        <div class="zoom_01">{{ Html::image('uploads/'.$macherKhaddo->image, null, ['class' => 'center-block', 'width' => 160, 'height' => 160]) }}</div>
                        <h4 class="cause-title fix"><label>{{ $macherKhaddo->name }}</label></h4>
                        <ul style="display: flex; justify-content: space-between; padding: 5px; margin-bottom: -15px; margin-top: -15px;">
                            <li style="list-style: none;" class="cause-title fix">{{ $macherKhaddo->quantity }}</li>
                            <li style="list-style: none;" class="cause-title fix">{{ $macherKhaddo->price }}</li>
                        </ul>


                        <div class="btn-holder text-center">

                            <button class="btn btn-primary" data-toggle="modal" data-target="#donateModal" id="gobadi-poshor-khaddo" value="{{ $macherKhaddo->id }}"><strong>বিস্তারিত</strong></button>
